...
async function fetchLastWeekPicks(username, poolName) {
    try {
        const response = await fetch(`https://www.pick6.club/api/getLastWeekPicks/${encodeURIComponent(username)}/${encodeURIComponent(poolName)}`);
...
async function handleAllPoolsView(username) {
    ...
    const poolPicks = await Promise.all(classicPools.map(async pool => {
        try {
            const response = await fetch(`https://www.pick6.club/api/getPicks/${username}/${pool.name}`);
            const data = await response.json();
            return {
                poolName: pool.name,
                picks: data.picks || [],
...
async function checkIfPlayoffPool(poolName) {
    try {
        const response = await fetch(`https://www.pick6.club/api/isPlayoffPool/${encodeURIComponent(poolName)}`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        ...
        if (classicPools.length === 1) {
            ...
            try {
                const lastWeekEndpoint = pool.isPlayoffPool ? 
                    `https://www.pick6.club/api/getLastWeekPlayoffPicks/${encodeURIComponent(storedUsername)}/${encodeURIComponent(pool.name)}` :
                    `https://www.pick6.club/api/getLastWeekPicks/${encodeURIComponent(storedUsername)}/${encodeURIComponent(pool.name)}`;
            ...
            if (hasPlayoffPool && hasRegularPool) {
                ...
                // For playoff pools, check if playoff pools have matching picks
                    ...
                    const currentPoolPicks = await Promise.all(playoffPools.map(async (pool) => {
                        try {
                            const response = await fetch(`https://www.pick6.club/api/getPlayoffPicks/${storedUsername}/${pool.name}`);
                            const data = await response.json();
                            return {
                                poolName: pool.name,
                                picks: data.picks || [],
                    ...
                    if (currentPicksMatch) {
                        ...
                        const lastWeekPoolPicks = await Promise.all(playoffPools.map(async (pool) => {
                            try {
                                const response = await fetch(`https://www.pick6.club/api/getLastWeekPlayoffPicks/${storedUsername}/${pool.name}`);
                                const data = await response.json();
                                return {
                                    poolName: pool.name,
                                    picks: data.success && Array.isArray(data.picks) ? data.picks : [],
        ...
        if (poolSelection === 'all') {
            ...
            // If we have only playoff pools or only regular pools
                ...
                const endpoint = isPlayoffMode
                    ? `https://www.pick6.club/api/getPlayoffPicks/${username}/${firstPool.name}`
                    : `https://www.pick6.club/api/getPicks/${username}/${firstPool.name}`;
...
async function handleSinglePoolView(username, poolSelection) {
    ...
    const endpoint = isPoolPlayoff 
        ? `https://www.pick6.club/api/getPlayoffPicks/${username}/${poolSelection}`
        : `https://www.pick6.club/api/getPicks/${username}/${poolSelection}`;
...
async function submitToPlayoffPool(poolName, data) {
    const response = await fetch(`https://www.pick6.club/api/savePlayoffPicks/${storedUsername}/${poolName}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
async function checkPlayoffEligibility(username, poolName) {
    try {
        // First, check if this is a playoff pool
        const playoffResponse = await fetch(`https://www.pick6.club/api/playoffs/isPlayoff/${encodeURIComponent(poolName)}`);
        ...
        const bracketResponse = await fetch(`https://www.pick6.club/api/playoffs/${encodeURIComponent(poolName)}/bracket`);
